...
static void print_example(int argc, char **argv){
	...
	printf("EXAMPLE: \n");
	printf("\n");
	printf("    %s --port /dev/ttyWCH0  --motor 1 --current 2.0 --step_mode 16 --ocd_th 3\n", argv[0]);
	printf("\n");
	printf("    Use serial port /dev/ttyWCH0 to configure motor 1\n");
	...
	printf("\n");
	printf("    %s -p /dev/ttyWCH0 -m 1 -c 2.0 -s 16 -d 3\n", argv[0]);
	printf("\n");
	printf("    Use serial port /dev/ttyWCH0 to configure motor 1\n");
	printf("    with drive current to 2.0A, step mode to 16 steps and\n");
	printf("    over detection current detection to 3A\n");
...
static void print_usage(int argc, char **argv){
	...
	printf("    -d, --device       serial port device name, default is /dev/ttyWCH0\n");
